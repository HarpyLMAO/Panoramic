    ...
    public void execute(CommandEvent command, TextChannel textChannel, Member member, String[] args, GuildMessageReceivedEvent event) {
        ...
        if (args.length == 0) {
            ...
            EmbedBuilder ownProfileEmbed = new EmbedBuilder()
                    ...
                    .setAuthor(member.getUser().getName() + "'s profile.", null, member.getEffectiveAvatarUrl())
            ...
            User targetUser = Bot.getInstance().getUserManager().getUserObjectRepository().find(args[0].replace("<@!", "").replace(">", ""));
            Member targetMember = command.getGuild().retrieveMemberById(args[0].replace("<@!", "").replace(">", "")).complete();

            if (targetUser == null) return;

            EmbedBuilder targetProfileEmbed = new EmbedBuilder()
                    .setColor(Bot.getInstance().getPanoramic().getColorColored())
                    .setAuthor(targetMember.getUser().getName() + "'s profile.", null, targetMember.getEffectiveAvatarUrl())
