...
public class BassBoostCommand implements BaseCommand {
    ...
    public void execute(CommandEvent command, TextChannel textChannel, Member member, String[] args, GuildMessageReceivedEvent event) {
        ...
        if (Integer.parseInt(input) == 50|| Integer.parseInt(input) == 0) {
            ...
            textChannel.sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("Bassboost has been disabled.").build()).queue();
            return;
        }

        if (Integer.parseInt(input) >= 401 || Integer.parseInt(input) <= -1) {
            textChannel.sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("The bassboost is out of range! [0-500]").build()).queue();
            return;
        }
        ...
        textChannel.sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("Bassboost has been applied, at the level " + input + "%.").build()).queue();
    }
