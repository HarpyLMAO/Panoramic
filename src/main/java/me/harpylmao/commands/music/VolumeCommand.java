...
public class VolumeCommand implements BaseCommand {
    ...
    public void execute(CommandEvent command, TextChannel textChannel, Member member, String[] args, GuildMessageReceivedEvent event) {
        ...
        if (Integer.parseInt(args[0]) >= 401 || Integer.parseInt(args[0]) <= -1) {
            textChannel.sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("The volume is out of range! [0-400]").build()).queue();
            return;
        }
        ...
        try {
            ...
            textChannel.sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("Please specify a volume!").build()).queue();
        } catch (NumberFormatException e) {
            textChannel.sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("That is not a number!").build()).queue();
        }

        textChannel.sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("🔈 Set volume to " + args[0] + "!").build()).queue();
    }
