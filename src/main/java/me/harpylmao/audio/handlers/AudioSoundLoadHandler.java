  ...
  public void trackLoaded(AudioTrack audioTrack) {
    ...
    if (!trackScheduler.getTrackQueue().isEmpty()) {
      EmbedBuilder embed = new EmbedBuilder()
        ...
        .addField(
          "Position in Queue",
          String.valueOf(trackScheduler.getTrackQueue().size()),
          true
        )
        .setFooter(
          "Added by " + member.getUser().getAsTag(),
    ...
    if (finalTrackURL.contains("ytsearch: ")) {
      ...
      if (sendMessage) {
        if (!trackScheduler.getTrackQueue().isEmpty()) {
          EmbedBuilder embed = new EmbedBuilder()
            ...
            .addField(
              "Song Duration",
              MusicUtils.getDuration(playlist.getTracks().get(0)),
              true
            )
            .addField(
              "Position in Queue",
      ...
      event
        ...
        .sendMessageEmbeds(
          new EmbedBuilder()
            .setColor(Bot.getInstance().getPanoramic().getColorColored())
            .setDescription("Loading playlist `" + playlist.getName() + "`")
            .build()
        )
        .queue();
