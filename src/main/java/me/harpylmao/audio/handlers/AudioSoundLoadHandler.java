package me.harpylmao.audio.handlers;
...
import me.harpylmao.utils.MusicUtils;
...
public class AudioSoundLoadHandler implements AudioLoadResultHandler {
	...
	public void trackLoaded(AudioTrack audioTrack) {
		...
		if (!trackScheduler.getTrackQueue().isEmpty()) {
			...
			if (sendMessage) {
				event.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription(":white_check_mark: **" + audioTrack.getInfo().title + "** successfully added to the queue!").build()).queue();
			...
			if (sendMessage) {
				event.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("Song added to queue.").build()).queue();
			}
	...
	public void playlistLoaded(AudioPlaylist playlist) {
		if (finalTrackURL.contains("ytsearch: ")) {
			...
			event.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("Loading playlist `" + playlist.getName() + "`").build()).queue();
	...
	public void noMatches() {
		...
		event.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Bot.getInstance().getPanoramic().getColorColored()).setDescription("Could not find song!").build()).queue();
	}
